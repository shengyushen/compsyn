...
(* 		self#relational_code  *)
		let (b , infered_assertion_array_lst_old_loop ) = self#findBound 
		in
		self#partition b infered_assertion_array_lst_old_loop
	end

	method partition b infered_assertion_array_lst_old_loop = begin
	...
	method findBound  = begin
		...
		in begin
			while (!stop) = false do
				b := (!b) +1 ;
				let (result_p, infered_assertion_array_lst_new_nonloop )= self#inferSATFormula_plr_nonloop (!b) (!b) (!b) (*(!infered_assertion_array_lst_old_nonloop)*) [] (!infered_assertion_array_lst_old_loop) 
				in 
				if(result_p = UNSATISFIABLE) then begin
					...
					let (result_p_loop, infered_assertion_array_lst_new_loop)= self#inferSATFormula_plr_loop (!b) (!b) (!b) (*(!infered_assertion_array_lst_old_nonloop)*) [] (!infered_assertion_array_lst_old_loop) (*in this case I dont need to encode common again*)
					in begin
			...
			;
			((!b),(!infered_assertion_array_lst_old_loop))
		end
	end
	...
	method genDecoderFunction iv shift ov instCNF = begin
		...
		in
		List.map (fun x -> (fst x, (simplify_withBDD (snd x) ddM))) ib_itpo_lst
	end
	...
	= begin
		...
		if(res == UNSATISFIABLE) then 
			...
			let listUniqBitI_shifted = List.map (fun x -> x+(p+l)*final_index_oneinst)  bv_instrlist
			in
	...
	method diffInput p l r = begin
		self#set_unlock_multiple;
		(*forcing the input to be not equal*)
		let ov = bv_instrlist
		in 
		let ov_0 = List.map (fun x -> x+ (p+l)*final_index_oneinst) ov
		and ov_b = List.map (fun x -> x+ (p+l+1+r+p+l)*final_index_oneinst) ov
		in
		let clslst = self#encode_INEV (ov_0) (ov_b)
		in begin
			self#set_lock_multiple;
			clslst
		end
	...
	method construct_varlst2assumption (p:int) (l:int) (r:int)  = begin
		...
		let nonuniq =bv_non_proctocol_input_list in
		let nonuniq_pl = List.map (fun x -> x+(p+l)*final_index_oneinst        ) nonuniq
		in
		initial_state_index_list @ initial_state_index_list2 @ ov_all @ nonuniq_pl 
	end
