...
and allsat_interp clslst_R target important_varlst non_important_varlst ddM= begin
	allsat_interp_BDD clslst_R target important_varlst non_important_varlst ddM true
end
and allsat_interp_noBDD clslst_R target important_varlst non_important_varlst ddM= begin
	allsat_interp_BDD clslst_R target important_varlst non_important_varlst ddM false 
end
and allsat_interp_BDD clslst_R target important_varlst non_important_varlst ddM simplifyOrnot= begin
	...
	(*a loop to infer*)
	let start_time_p3 = Unix.gettimeofday () in
		...
		while ((!res)!=UNSATISFIABLE) do
			...
			| _ -> begin
				...
				in begin
					...
					in begin
					dbg_print (sprintf "  after characterization_interp");
					let new_assertion = begin
						if(simplifyOrnot) then begin
							let res=simplify_withBDD new_assertion_preBDD ddM
							in begin
								dbg_print "  TIME of simplify_withBDD";
								res
							end
						end
					...
					in begin
						dbg_print (sprintf "  after simplify_withBDD");
						...
						;
						dbg_print (sprintf "  after force_assertion_alone %d" (List.length (!clslst_R_new)));
			...
			;
			(*coming to here and with non_important_varlst == [] 
			means we dont need to check the satisfiability anymore, 
			because all cases that leads to SAT have been inferred already*)
