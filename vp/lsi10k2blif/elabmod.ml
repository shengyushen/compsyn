	...
	method compsyn (delay:int) (length:int) (instrlist:string list) (outstrlist:string list) = begin
		...
		in begin
			...
			*)
			...
			let bound_total = delay+length
			in
			let proc_try_delay_length_prefix_forward d l p f= begin
				...
				in begin
					if satres11==SATISFIABLE then begin
						...
						begin
							...
							fprintf continue_scr "%s\n"  rm_cmd
							;
							let nl_const_cmd=String.concat "" ["nl_const " ; nl_const_cnfname ; " 100 |tee nl_const.log"] (*use 100s as timeout value, to prevent us from stuck at a hopeless long run*)
							in
							fprintf continue_scr "%s\n"  nl_const_cmd
							;
			...
			in
			let rec proc_try_delay_length_prefix d l p f= begin(*track forward*)
				if f > p then (SATISFIABLE,0,0,0,0)
				else begin
					let (res,md,ml,mp,mf)=proc_try_delay_length_prefix_forward d l p f
					in begin
						if(res==SATISFIABLE) then 
							proc_try_delay_length_prefix d l p (f+1)
						else
							(res,md,ml,mp,mf)
					end
				end
			...
			in
			let rec proc_try_len_delay bd len dd= begin
				if ((len+dd) > bd) then (SATISFIABLE,0,0,0,0)
				else begin
				  let prefix=bd-len-dd
				  in begin
					let (res,md,ml,mp,mf)=proc_try_delay_length_prefix dd len prefix 0
					in begin
						if (res==SATISFIABLE) then 
							proc_try_len_delay bd len (dd+1)
						else 
							(res,md,ml,mp,mf)
					end
				  end
			...
			in
			let rec proc_try_bound bd = begin
				if (bd > bound_total) then (SATISFIABLE,0,0,0,0)
				else begin
					...
					let (res,md,ml,mp,mf)=proc_try_len bd 1
					in begin
						if (res==SATISFIABLE) then  (*that means exist multiple reverse solution, we need to search further*)
							proc_try_bound (bd+1)
						else (*found solution*)
