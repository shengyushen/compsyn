...
object (self)
	...
	*)
	...
	method compsyn (*(bound_dl:int)*) (instrlist1:string list) (outstrlist1:string list) = begin
		...
		begin
			...
			*)
			...
			in
			let (pres,p_pre,l,r,infered_assertion_array_lst_new_nonloop,infered_assertion_array_lst_new_loop)=self#try_bound_wrap
			in begin
				...
				let infered_assertion_array_lst_new_nonloop1=List.map (fun x -> simplify_withBDD x ddM) infered_assertion_array_lst_new_nonloop
				in 
				let infass= begin
					if (List.length infered_assertion_array_lst_new_nonloop1)!=0 then 
						simplify_withBDD (invert_assertion (or_assertion infered_assertion_array_lst_new_nonloop1)) ddM
					else 
						Array.make 1 TiterpCircuit_true
				end
				in begin
					printf "the final assertion :\n";
					printf "++++++++++++++++++++++++++\n";
				...
				if(pres==RES_UNIQ) then begin
					...
					let d1=self#reduce_d p_pre l r infered_assertion_array_lst_new_nonloop
					in
					let p1=self#reduce_p p_pre l d1 infered_assertion_array_lst_new_nonloop
					in
					let l1=self#rmred (p1+l) 1 d1 infered_assertion_array_lst_new_nonloop(*here l means the left exp, while l1 mean output length*)
					in begin
						...
						dbg_print (sprintf "rmred p1 %d l1 %d d1 %d\n" p1 l1 d1);
						...
						in begin
							...
							assert(lock_oneinst==true);
							assert(lock_multiple==true);
							self#gen_relation_pld new_p1 l1 d1 infered_assertion_array_lst_new_nonloop;
							...
							dbg_print "compsyn 1";
							(*use the generated clause_list_multiple directly*)
							self#gen_dec_and_ia  new_p1 l1 d1 infered_assertion_array_lst_new_nonloop start_time_p2
						end
						;
						assert(lock_oneinst==true);
						assert(lock_multiple==true);
	...
	method try_bound_wrap = begin
		...
		in begin
			while ( (!res) != RES_UNIQ ) do
				b := (!b)+1 ;
				xxx := self#check_p_r_l (!b) (!b) (!b) (!infered_assertion_array_lst_old_nonloop) (!infered_assertion_array_lst_old_loop);
				match (!xxx) with
	...
	method check_p_r_l (p:int) (r:int) (l:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		(*sent two empty list to findLR_checkO_nonloop such that it can discover a whole new set to replace the current one*)
		let (result_p,infered_assertion_array_lst_new_nonloop)=self#findLR_checkO_nonloop p l r [] []
		in begin
			assert(lock_multiple==true);
			if (result_p==UNSATISFIABLE) then begin
				...
				let (result_p_loop,infered_assertion_array_lst_new_loop)=self#findLR_checkO_loop_wrap p l r infered_assertion_array_lst_new_nonloop infered_assertion_array_lst_old_loop (*in this case I dont need to encode common again*)
				in begin
	...
	method findLR_checkO_nonloop (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop=begin
		...
		flush stdout;
		let target = self#findLR_common p l r  infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop
		in begin
			(*dump to sat*)
			...
			in begin
				if(res==UNSATISFIABLE) then (res,infered_assertion_array_lst_old_nonloop) (*it is ok already*)
					...
					let (res1,itplst)=allsat_interp clause_list_multiple target bv_non_proctocol_input_list (self#construct_varlst2assumption p l r) ddM
					in begin
	...
	method findLR_checkO_loop_wrap (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop=begin
