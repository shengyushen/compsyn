	...
	*)
	...
	method alloc_and_equ_res ov_pair_lst = begin
		...
		(a, b) -> begin
			...
			in begin
				clause_list_multiple <- equres @ clause_list_multiple ;
				r
			end
		end
	end
	...
	method dump_sat = begin
		MiniSAT.reset ();
		let res=Dumpsat.dump_sat clause_list_multiple
		in 
		match res with
		Dumpsat.SATISFIABLE -> SATISFIABLE
		| Dumpsat.UNSATISFIABLE -> UNSATISFIABLE
	end
(*	method dump_sat = begin
		MiniSAT.reset ();
		let max_index=self#get_largest_varindex_inclslst clause_list_multiple
		in begin
	...
	*)	
	...
	method check_p_r_l (p:int) (r:int) (l:int) (instrlist:string list) (outstrlist:string list) = begin
		let result_p=self#findLR_checkO p l r instrlist outstrlist
		in begin
			if (result_p==UNSATISFIABLE) then (RES_UNIQ,p,l,r) (* output uniquely determine input*)
			else begin
				let result_p_loop=self#findLR_checkO_loop p l r instrlist outstrlist
				in begin
					if (result_p_loop==SATISFIABLE) then begin
						Printf.printf " RES_NONU\n";
						flush stdout;
						(RES_NONU,p,l,r) (* output uniquely determine input*)
					end
					else (RES_UNK,p,l,r)
				end
			end
	...
	method try_bound_p_r_l (b1:int) (p1:int) (r1:int) (l1:int) (instrlist:string list) (outstrlist:string list) = begin
		Printf.printf " try_bound_p_r_l : b1 %d  p1 %d r1 %d l1 %d\n" b1 p1 r1 l1;
		flush stdout;
		let (result_p,p_pre,l,r)=self#check_p_r_l p1 r1 l1 instrlist outstrlist
		in begin
			if (result_p==RES_UNIQ) then (RES_UNIQ,p_pre,l,r) (* output uniquely determine input*)
			else if (result_p==RES_NONU) then (RES_NONU,p_pre,l,r)
			else if (p1+r1+l1<b1-1) then  begin
				self#try_bound_p_r_l b1 p1 r1 (l1+1) instrlist outstrlist
			end
			else (RES_UNK,0,0,0)
		end
	end

	method try_bound_p_r (b1:int) (p1:int) (r1:int) (instrlist:string list) (outstrlist:string list) = begin
		(*Printf.printf " try_bound_p_r :   b1 %d  p1 %d r1 %d\n" b1 p1 r1;
		flush stdout;*)
		if (r1>b1-1-p1-2) then (RES_UNK,0,0,0)
		else begin
		let (result_p,p_pre,l,r)=self#try_bound_p_r_l b1 p1 r1 (b1-p1-r1-1) instrlist outstrlist
		in begin
			if (result_p==RES_UNIQ) then (RES_UNIQ,p_pre,l,r) (* output uniquely determine input*)
			else if (result_p==RES_NONU) then (RES_NONU,p_pre,l,r)
			else if (r1<b1-1-p1-2) then  begin
				self#try_bound_p_r b1 p1 (r1+1) instrlist outstrlist
			end
			else (RES_UNK,0,0,0)
		end
		end
	...
	method try_bound_p (b1:int) (p1:int) (instrlist:string list) (outstrlist:string list) = begin
		(*Printf.printf " try_bound_p :     b1 %d  p1 %d\n" b1 p1;
		flush stdout;*)
		let (result_p,p_pre,l,r)=self#try_bound_p_r b1 p1 2 instrlist outstrlist
		in begin
			if (result_p==RES_UNIQ) then (RES_UNIQ,p_pre,l,r) (* output uniquely determine input*)
			else if (result_p==RES_NONU) then (RES_NONU,p_pre,l,r)
			else if (p1<b1-1-2-2) then  begin
				self#try_bound_p b1 (p1+1) instrlist outstrlist
			end
			else (RES_UNK,0,0,0)
		end
	end
	...
	method try_bound  (b1:int) (instrlist:string list) (outstrlist:string list)= begin
		Printf.printf " try_bound :       b1 %d  \n" b1 ;
		flush stdout;
		let (result_p,p_pre,l,r)=self#check_p_r_l b1 b1 b1 instrlist outstrlist
		in begin
			if (result_p==RES_UNIQ) then (RES_UNIQ,p_pre,l,r) (* output uniquely determine input*)
			else if (result_p==RES_NONU) then (RES_NONU,p_pre,l,r)
			else begin
				self#try_bound (b1+1) instrlist outstrlist
			end
		end
	end

	method findLR_common (p:int) (l:int) (r:int) (instrlist:string list) (outstrlist:string list)=begin
	...
	method findLR_dumpSAT (p:int) (l:int) (r:int) (instrlist:string list) (outstrlist:string list)=begin
		self#dump_sat
		(* dont pass clause_list_multiple *)
		(*self#dump_sat clause_list_multiple*)
	end
		...
		in begin
			...
			let proc_liequ li = begin
				...
				let proc_riequ ri = begin
					...
					in
					let res_lst = List.map (self#alloc_and_equ_res) ov_pair_lst
					in
					...
					in
					let res_lst_shift = List.map (self#alloc_and_equ_res) ov_pair_lst_shift
					and rii = self#alloc_index 1
					in begin
						clause_list_multiple <- (self#encode_Red_AND rii (res_lst @ res_lst_shift)) @ clause_list_multiple ;
						rii
					end
				end
				in
			...
			in begin
				...
				clause_list_multiple <- ([riii],"")::clause_list_multiple ;
				riii
			end
		end
	end
	
	method double_loop_l (p:int) (l:int) (r:int) = begin
		let dff_idxpairlst=List.filter (self#isdff_name_index) name_index_lst
		in
		let dff_curlst=List.map (fun x -> match x with (_,(y,_)) -> y) dff_idxpairlst
		in begin
			let lilst=self#lr2list p (p+l-1)
			in
			let proc_liequ li = begin
				let ov_li  = List.map (fun x -> x+ li*final_index_oneinst) dff_curlst
				and rilst=self#lr2list (li+1) (p+l)
				in
				let proc_riequ ri = begin
					let ov_ri  = List.map (fun x -> x+ ri*final_index_oneinst) dff_curlst
					in
	...
	method findLR_checkO_loop (p:int) (l:int) (r:int) (instrlist:string list) (outstrlist:string list)=begin
		(*Printf.printf " findLR_checkO_loop : p %d l %d r %d\n" p l r;
		flush stdout;*)
		if(p<2 || l<2 || r<2) then UNSATISFIABLE
		else begin
				(* remove this to avoid  findLR_common again,
				must make sure that findLR_checkO_loop follows findLR_checkO *)
				(*self#findLR_common p l r instrlist outstrlist ;*)
				self#double_loop_p p l r;
				self#double_loop_l p l r;
				self#double_loop_r p l r;
				(*both side can detect loop*)

				self#findLR_dumpSAT p l r instrlist outstrlist
		end
	end
	
	method findLR_checkO (p:int) (l:int) (r:int) (instrlist:string list) (outstrlist:string list)=begin
	...
	method rmred_check (p:int) (l:int) (d:int) (instrlist:string list) (outstrlist:string list)=begin
		clause_list_multiple <- [];
		last_index <- final_index_oneinst;
		...
		*)
		...
		(*dump to sat*)
		self#findLR_dumpSAT p l d instrlist outstrlist ;
	end

	
	
	method reduce_p (p:int) (l:int) (d:int) (instrlist:string list) (outstrlist:string list) =begin
		if(p==0) then p
		else begin
			let res=self#findLR_checkO (p-1) l d instrlist outstrlist
			in begin
				if(res==UNSATISFIABLE) then self#reduce_p (p-1) l d instrlist outstrlist
				else p
			end
		end
	end
	
	method reduce_d (p:int) (l:int) (d:int) (instrlist:string list) (outstrlist:string list) =begin
		if(d==0) then d 
		else begin
			let res=self#findLR_checkO p l (d-1) instrlist outstrlist
			in begin
				if(res==UNSATISFIABLE) then self#reduce_d p l (d-1) instrlist outstrlist
				else d
			end
		end
	end
	
	method rmred (p:int) (l:int) (d:int) (instrlist:string list) (outstrlist:string list) =begin
		let resO  =self#rmred_check p l d instrlist outstrlist
		in begin
			if (resO==UNSATISFIABLE) then l
			else self#rmred p (l+1) d instrlist outstrlist
		end
	end
	
	(*p means the prefix , l1 means the length of output, r1 means the delay*)
	method gen_characterizer  (p1:int) (l1:int) (r1:int) (instrlist:string list) (outstrlist:string list) =begin
		let bd=p1+1+r1
		and d=r1
		and p=p1
		and f=l1
...
(* 		and cnfname_dualsyn=String.concat "" [tempdirname ; "dumpout/dualsyn_" ; name ;".cnf"] *)
		...
		begin
			...
			*)
			...
			in
			let (pres,p_pre,l,r)=self#try_bound 2 instrlist outstrlist
			in begin
			  ...
			  if(pres==RES_UNIQ) then begin
				...
				let d1=self#reduce_d p_pre l r instrlist outstrlist
				in
				let p1=self#reduce_p p_pre l d1 instrlist outstrlist
				in
				let l1=self#rmred (p1+l) 1 d1 instrlist outstrlist (*here l means the left exp, while l1 mean output length*)
				in begin
					Printf.printf "rmred p1 %d l1 %d d1 %d\n" p1 l1 d1;
					...
					self#gen_characterizer (p1+l) l1 d1 instrlist outstrlist;(*l shound not be changed*)
	...
	method gen_instance inst bound= begin
		if inst == bound then []
		else begin
			let oclslst=self#gen_instance (inst+1) bound
			in
			(self#encode_instance inst) @ oclslst
		end
	end

	(*generate multiple instance, which bound is the number*)
	method gen_multiple_instance_step2 num = begin
		let clslst=self#gen_instance 0 num
		in 
		(clslst,(final_index_oneinst*num))
	end
	...
	*)
		...
		T_lvalue_id([name]) -> begin
			let baseidx = begin
				...
				in begin
					...
					(*non-dff items have -1 in its x2*)
	...
	method check_intlst_dump intlst = begin
		match intlst with
		[] -> false
		| hd::tl -> begin
			if (List.mem hd tl) then begin
				printf "FATAL : duplicate literal\n";
				self#prtcls intlst;
				true
			end
			else self#check_intlst_dump tl	
		end
	end

	method check_cls_dump cls = begin
		match cls with
		(intlst,comment) -> begin
			let res=self#check_intlst_dump intlst
			in begin
	...
	method check_cnf clslst = begin
		List.iter (self#check_cls_dump) clslst
	end
	
	(*print_cnf*)
	method print_cnf clslst dumpout_cnf dly len prefix forward instrlist outstrlist= begin
		self#check_cnf clslst;
	...
	method prtcls cls = begin
		List.iter (fun x -> printf "%d " x) cls;
		printf "\n"
	end

	(*method prtclslst clslst = begin
		let clslst_nostr=List.map (fun x -> match x with (intlst,_) -> intlst) clslst
		in begin
			printf "+++++++++++start+++++++++++\n";
			List.iter (self#prtcls) clslst_nostr;
			printf "++++++++++++end++++++++++++\n"
		end
	end*)
end
