...
object (self)
	...
	*)
	...
	method procDetermineUniqueInputs inlst = begin
		...
		let procBitnonloop p l r idx x  = begin
				...
				RES_UNK -> begin
					...
					let nonloopres = satAssumption [-o0;ob] in begin	
						if(nonloopres == UNSATISFIABLE) then begin
							...
							printf "uniq %s @ %d\n" name bitI;
							flush stdout;
							Array.set arrlist2 idx (bitI,RES_UNIQ,p,l,r);
						end
					end
				end
				| _ -> ()
		...
		and procBitloop p l r idx x  = begin
		...
		in begin
			...
			while((self#arrayexists (fun x -> match x with (_,res,_,_,_) -> (res == RES_UNK)) arrlist2)) do
				...
				(*nonloop case*)
				(**********************)
				let target_nonloop = self#construct_nonloop (!p) (!l) (!r) [] [] in 
				let nonloop_clslst=(([target_nonloop],"target")::clause_list_multiple) in begin
					...
					Array.iteri (procBitnonloop (!p) (!l) (!r)) arrlist2;
				end;
				(**********************)
				(*loop case*)
				(**********************)
			...
			List.map (fun x -> match x with (bitii,res,p,l,r) -> assert (res!= RES_UNK);((res == RES_UNIQ),bitii,p,l,r)) lstres
		end
	...
	method compsyn (*(bound_dl:int)*) (instrlist1:string list) (outstrlist1:string list) = begin
		...
		let (listUniqBitITmp,listNonuniqBitITmp) = begin
			...
			List.partition (fun x -> match x with (tf,_,_,_,_) -> tf) list_tf_BitI_n
		end in
		let maxp = begin
			if((List.length listUniqBitITmp)==0) then begin
				dbg_print "Nothing uniquely determined, decoder not exits";
				exit 0;
			end;
			(*find out the largest one that make the flow control vars to be uniquely determind*)
			let listN = List.map (fun b -> match b with (_,_,n,_,_) -> n) listUniqBitITmp in
			Intlist.listMax listN
		end(*find out the bit index*)
		and maxl = begin
			(*find out the largest one that make the flow control vars to be uniquely determind*)
			let listN = List.map (fun b -> match b with (_,_,_,n,_) -> n) listUniqBitITmp in
			Intlist.listMax listN
		end(*find out the bit index*)
		and maxr = begin
			(*find out the largest one that make the flow control vars to be uniquely determind*)
			let listN = List.map (fun b -> match b with (_,_,_,_,n) -> n) listUniqBitITmp in
			Intlist.listMax listN
		end(*find out the bit index*)
		and listUniqBitI = List.map (fun x -> match x with (_,bit,_,_,_) -> bit) listUniqBitITmp 
		and listNonuniqBitI = List.map (fun x -> match x with (_,bit,_,_,_) -> bit) listNonuniqBitITmp in 
		let (pres,
				p_pre,
				l1,
				r1,
		...
		(*****************************************)
			...
			self#inferPredicateUniq listUniqBitI listNonuniqBitI  maxp maxl maxr
		end in 
		let finalass = begin
		(*****************************************)
		(*simplifying valid(f)*)
		...
		let decgen1 lst = begin
			...
			List.for_all (fun x -> match x with (tf,_,_,_,_) -> tf) lst;
			let iv=List.map (fun x -> match x with (_,idx,_,_,_)-> idx) lst in
	...
	method inferPredicateUniq listUniqBitI listNonuniqBitI maxp maxl maxr = begin
		self#inferSATFormula_plr 
									maxp
									maxl
									maxr
									[] 
