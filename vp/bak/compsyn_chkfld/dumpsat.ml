...
and encode_EQU_const_res idx1 v res = begin
	assert (idx1 >0);
	match v with
	0 -> ([-idx1],"")
	| 1 -> ([idx1],"")
...
and encode_NEG_list idxl1 idxl2 tmplst = begin
	let len1 = List.length idxl1
	and len2 = List.length idxl2
	and lentmp = List.length tmplst
	in begin
		assert (len1=len2);
		assert (len1=lentmp);
		let comblst = List.combine (List.combine idxl1 idxl2 ) tmplst
		in
		let clsList = List.concat (List.map (fun x ->  encode_NEG_res (fst (fst x)) (snd (fst x)) (snd x) ) comblst)
		in
		(tmplst,"reduceOR_NEG_list")::clsList
	end
end
and encode_NEG_res idx1 idx2 tmp = begin
	[([-idx1;-idx2;-tmp],"");([idx1;idx2;-tmp],"");([-idx1;idx2;tmp],"");([idx1;-idx2;tmp],"")]
end
