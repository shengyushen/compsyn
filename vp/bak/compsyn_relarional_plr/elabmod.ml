...
object (self)
	...
	*)
	...
	method minimizeLR p l r assLst = begin
		...
		dbg_print "minimizeLR ";
		let newl=self#minimizeL p l r assLst in
		let newr=self#minimizeR p newl r assLst in begin
			dbg_print "exiting minimizeLR";
			(newl,newr)
		end
	end
	...
	method compsyn (*(bound_dl:int)*) (instrlist1:string list) (outstrlist1:string list) = begin
		...
		let (pres,
				...
				l1,
				r1,
				infered_assertion_array_lst_new_nonloop,
		...
		(*****************************************)
			...
			self#try_bound_wrap listUniqBitI listNonuniqBitI maxN 
		end in 
		let finalass = begin
		(*****************************************)
		(*simplifying valid(f)*)
		(*****************************************)
			dbg_print "exiting try_bound_wrap";
		...
		(*2014/11/11 Shen : why do we need this?
		  it will characterize the set of unique inputs that can make the inferred assertion and the specified assertion true 
		  but the latter one can and will be enforced seperately*)
		(*let finalass_conjunct = self#conjunctInfAssertionWithTR finalass listUniqBitI in
		let (l,r)=self#minimizeLR p_pre (l1+1) (r1+1) [finalass_conjunct] in begin*)
		let (l,r)=self#minimizeLR p_pre (l1+1) (r1+1) [finalass] in 
		let instCNF = begin
		(*****************************************)
		...
		(*****************************************)
			...
			printf "\n++++++++++++++++++++++++++\n";
			dbg_print "time of inferring predicate" ;
		...
		let decgen x = begin
			match x with 
			(true,idx,_)->    begin
				(*flow control vars that can be uniquely determined, so dont use valid(f)*)
				printf "true idx %d nm %s\n" idx (self#idx2name idx);
				let itp = self#genDecoderFunction (idx+(p_pre+l)*final_index_oneinst) ov instCNF in 
				(idx,itp)
			end
			|(false,idx,_) -> begin
				(*data vars that need valid(f)*)
				printf "false idx %d nm %s\n" idx (self#idx2name idx);
				let itp = self#genDecoderFunction (idx+(p_pre+l)*final_index_oneinst) ov instCNF2 in 
				(idx,itp)
			end
		end in
		let idx2decfunList = List.map decgen list_tf_BitI_n in begin
		(*****************************************)
		(*characterizing all input vars decoder function*)
		(*****************************************)
			self#generateDecoderVerilog p_pre l r idx2decfunList;
			dbg_print "succesfully generate the decoder with time ";
		end
	end

	method genDecoderFunction idx ov clslst  = begin
		dbg_print "genDecoderFunction";
		let (res1,itplst)= allsat_interp_noBDD clslst idx ov []  ddM in begin
		...
		in begin
			...
			let prtFunction idx2decfun = begin
				...
				(idx,decfun) -> begin
					...
					self#print_itpo_verilog_file decoderVerilog decfun idx;
				end
			end in
	...
	method conjunctInfAssertionWithTR finalass listUniqBitI = begin
		...
		dbg_print "conjunctInfAssertionWithTR";
					...
					let allinputs = bv_instrlist@bv_non_proctocol_input_list in begin
						...
						let (res1,itplst)= allsat_interp clause_list_multiple var_assertion listUniqBitI allotherinputs ddM in begin
							...
							if (List.length itplst)!=0 then begin
								...
								(*2014/11/11 Shen : why should we use some complex one?*)
								simplify_withBDD (and_assertion [finalass;(or_assertion itplst)]) ddM
							end
							else begin
								Array.make 1 TiterpCircuit_true
							end
	...
	method try_bound_wrap listUniqBitI listNonuniqBitI maxN = begin
		...
		in begin
			while ( (!res) != RES_UNIQ ) do
				b := (!b)+1 ;
				dbg_print "pre self#check_p_r_l";
				...
				dbg_print "after self#check_p_r_l";
				match (!xxx) with
	...
	method check_p_r_l (p:int) (r:int) (l:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI = begin
		...
		in begin
			dbg_print "after check_p_r_l->findLR_checkO_nonloop";
...
*)
	...
	method construct_nonloop_1copy (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		...
		*)
		...
		let clslst =(self#connect_multiple_instance_step3 0 (p+l+r)) in begin
			dbg_print "construct_nonloop_1copy 4.5";
	...
	method construct_nonloop (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		...
		*)
		...
		dbg_print "construct_nonloop 4";
		(*connect the first p+1+r instance*)
		let clslst =(self#connect_multiple_instance_step3 0 (p+l+r)) in begin
			dbg_print "construct_nonloop 4.5";
			self#append_clause_list_multiple  clslst ;
		end;
		(*check_clslst_maxidx clause_list_multiple last_index;*)
		
		dbg_print "construct_nonloop 5";
		(*connect the secnod p+1+r instance*)
		let clslst2=(self#connect_multiple_instance_step3 (p+l+1+r) ((p+l+1+r)*2-1)) in begin
			dbg_print "construct_nonloop 5.5";
		...
		in begin
			dbg_print "construct_nonloop 10";
	...
	*)
	...
	method print_itpo_verilog_file fv arr_itpo idx = begin
		let size = Array.length arr_itpo in begin
			(*we are sure that 0 is not used*)
			for i= 1 to (size -1) do
					fprintf fv "assign w_%d_%d = " idx i;
					...
					let varstr=prt_itpo (Array.get arr_itpo i) in
					fprintf fv " %s ;\n" varstr ;
			done;
		end
	end
