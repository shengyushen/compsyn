...
object (self)
	...
	*)
	...
	method compsyn (*(bound_dl:int)*) (instrlist1:string list) (outstrlist1:string list) = begin
		...
		begin
			...
			*)
			...
			set_current_time;
			let start_time_check_comp = Unix.gettimeofday () in begin
				(*following is relational code*)
				let procBitIs inlst = begin
					let inlst2=List.map (fun x -> (x,RES_UNK,0,0,0)) inlst in
					let arrlist2=Array.of_list inlst2 
					and nonloop_clslst=ref [] 
					and loop_clslst=ref [] 
					and bnd=ref 2 in
					let procBitI idx x = begin
						...
						List.map (fun x -> match x with (bitii,res,p,l,r) -> assert ((p==l) && (l==r) && (res!=RES_UNK));((res==RES_UNIQ),bitii,p)) (Array.to_list arrlist2)
					end
				end in
				let list_tf_BitI_n= procBitIs bv_instrlist in begin
					let listUniqBitITmp=List.filter (fun x -> match x with (tf,_,_) -> tf) list_tf_BitI_n
					and listNonuniqBitITmp=List.filter (fun x -> match x with (tf,_,_) -> (tf!=true)) list_tf_BitI_n in
					let maxN=List.fold_left (fun a b -> match b with (_,_,n) -> (max a n)) 0 listUniqBitITmp 
					and listUniqBitI=List.map (fun x -> match x with (_,bit,_) -> bit) listUniqBitITmp 
					and listNonuniqBitI=List.map (fun x -> match x with (_,bit,_) -> bit) listNonuniqBitITmp 
					and fctime = Unix.gettimeofday () in begin
						printf "maxN %d\n" maxN;
						printf "time of finding flow control %f\n" (fctime -. start_time_check_comp);
						assert((List.length listUniqBitITmp)>0);
						let (pres,p_pre,l,r,infered_assertion_array_lst_new_nonloop,infered_assertion_array_lst_new_loop)=self#try_bound_wrap listUniqBitI listNonuniqBitI maxN in begin
							...
							let finalass_conjunct=self#conjunctInfAssertionWithTR finalass listUniqBitI
							and iftime = Unix.gettimeofday () in begin
								printf "the assertion before conjunct with TR\n";
								printf "++++++++++++++++++++++++++\n";
								self#print_itpo finalass;
								printf "\n++++++++++++++++++++++++++\n";
								printf "p_pre %d l %d r %d: \n" p_pre l r;
								printf "the assertion that make all uniq is:\n" ;
								printf "++++++++++++++++++++++++++\n";
								self#print_itpo finalass_conjunct;
								printf "\n++++++++++++++++++++++++++\n";
								printf "time of inferring predicate %f\n" (iftime -. fctime);
								self#construct_nonloop_1copy p_pre l r [] [];
								let instCNF=clause_list_multiple in begin
									self#construct_nonloop_1copy p_pre l r [finalass_conjunct] [];
									let instCNF2=clause_list_multiple in begin
										let lrlst=lr2list p_pre (p_pre+l+r) in
										let ov=List.concat (List.map (fun frm -> List.map (fun x -> x+frm*final_index_oneinst) bv_outstrlist) lrlst) in
										let decgen x = begin
											match x with 
											(true,idx,_)->    begin
												printf "true idx %d nm %s\n" idx (self#idx2name idx);
												let itp=self#genDecoderFunction (idx+(p_pre+l)*final_index_oneinst) ov instCNF in (idx,itp)
											end
											|(false,idx,_) -> begin
												printf "false idx %d nm %s\n" idx (self#idx2name idx);
												let itp=self#genDecoderFunction (idx+(p_pre+l)*final_index_oneinst) ov instCNF2 in (idx,itp)
											end
										end in
										let idx2decfunList=List.map decgen list_tf_BitI_n in
										self#generateDecoderVerilog p_pre l r idx2decfunList;

										let gendctime = Unix.gettimeofday () in
										printf "succesfully generate the decoder with time %f\n"  (gendctime -. iftime);
									end
								end
							end
						end
	...
	method genDecoderFunction idx ov clslst  = begin
		let (res1,itplst)=allsat_interp clslst idx ov []  ddM in begin
			assert (res1==UNSATISFIABLE);
			let finalass = begin
				if(List.length itplst)==0 then begin
					printf "only false idx %d nm %s\n" idx (self#idx2name idx);
					Array.make 1 TiterpCircuit_false
				end
				else if(List.length itplst)==1 then List.hd itplst
				else assert false
			end in
			finalass
		end
	...
	method bitnamelst2namerangelst bitlst= begin
		...
		List.map (fun x -> match x with (nm,[]) -> (nm,-1,-1) | (nm,vlst)-> let (l,r)=self#list2lr vlst in (nm,l,r)) nameidxlstlst 
	end

	method list2lr idxlst= begin
		...
		in begin
			...
			let prtbit x = begin
				...
				(nm,-1,r) -> begin
					...
					fprintf decoderVerilog "wire   cycle%d_%s=%s;\n" (prefix+left+right) nm nm
				end
				| (nm,l,r) -> begin
					assert(l!=(-1));
					assert(r!=(-1));
					fprintf decoderVerilog "input [%d:%d]  %s;\n" l r nm;
					fprintf decoderVerilog "wire  [%d:%d]  cycle%d_%s=%s;\n" l r (prefix+left+right) nm nm
				end
			end in
			List.iter prtbit bvo;
			...
			let prtFunction idx2decfun = begin
				...
				(idx,decfun) -> begin
					fprintf decoderVerilog "assign %s=" (self#idx2name idx) ;
					self#print_itpo_verilog_file decoderVerilog decfun;
					fprintf decoderVerilog ";\n\n"
				end
			end in
			List.iter prtFunction idx2decfunList;
	...
	method conjunctInfAssertionWithTR finalass listUniqBitI= begin
		(*constrain the assertion wire*)
		if (List.length (List.filter (self#is_assertion) name_index_lst)) > 0 then begin
			check_itpo_var_membership finalass listUniqBitI;
			self#set_unlock_multiple;
		
			self#set_clause_list_multiple [];
			self#set_last_index final_index_oneinst;

			let (clause_list_multiple_aux,last_index_aux) = (self#gen_multiple_instance_step2 1)
			in begin
			...
			self#force_assertion finalass;

			self#set_lock_multiple;
			let var_assertionlst=self#name2idxlist assertion_shengyushen in begin
				match var_assertionlst with 
				[var_assertion] -> 
					let allinputs=bv_instrlist@bv_non_proctocol_input_list in begin
						assert(Intlist.isLst1inLst2 listUniqBitI allinputs);
						let allotherinputs=Intlist.sublst allinputs listUniqBitI in
						let (res1,itplst)=allsat_interp clause_list_multiple var_assertion listUniqBitI allotherinputs ddM in begin
							assert (res1==UNSATISFIABLE);
							if (List.length itplst)!=0 then 
								simplify_withBDD (and_assertion [finalass;(simplify_withBDD (or_assertion itplst) ddM)]) ddM
							else begin
								Array.make 1 TiterpCircuit_true
							end
						end
				...
				|_ -> begin
					printf "too many assertions";
					assert false;
				end
			end
		end
		else begin
			dbg_print "warning : not found assertion_shengyushen";
			finalass
		end
	end

	method try_bound_wrap listUniqBitI listNonuniqBitI maxN= begin
		assert (maxN>=2);
		let b = ref (maxN-1)
		and res = ref RES_UNK
		...
		in begin
			while ( (!res) != RES_UNIQ ) do
				b := (!b)+1 ;
				xxx := self#check_p_r_l (!b) (!b) (!b) (!infered_assertion_array_lst_old_nonloop) (!infered_assertion_array_lst_old_loop) listUniqBitI listNonuniqBitI ;
				match (!xxx) with
				...
				;
				(*now it is time to check the non-input dependence*)
				(*let res_noninput=self#check_noninput_dependence (!b) (!infered_assertion_array_lst_old_nonloop) (!infered_assertion_array_lst_old_loop)
				in begin
					if(res_noninput==true) then begin
						(*the output and state only depend on state and config
						so the input can't be recovered from the output*)
						printf "INFO : the output and state do not dependent on input\n";
						res := RES_UNIQ ;
					end
				end*)
			done
	...
	method check_noninput_dependence p infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		self#set_unlock_multiple;
		
		self#set_clause_list_multiple [];
		self#set_last_index final_index_oneinst;
		
		(*generate 0 to p instance*)
		let (clause_list_multiple_aux,last_index_aux) = self#gen_multiple_instance_step2 (p+1)
		in begin
			self#set_clause_list_multiple clause_list_multiple_aux;
			self#set_last_index last_index_aux;
			()
		...
		(*constrain the assertion wire*)
		if (List.length (List.filter (self#is_assertion) name_index_lst)) > 0 then begin
			self#append_clause_list_multiple (self#try_assertion 0 p);
		end
		else begin
			dbg_print "warning : not found assertion_shengyushen";
		end
		...
		(*connect the first p+1 instance*)
		self#append_clause_list_multiple  (self#connect_multiple_instance_step3 0 p) ;
		check_clslst_maxidx clause_list_multiple last_index;
		
		(*force the npi to be the same*)
		let npi_lst=bv_non_proctocol_input_list
		and npi_cycle_lst = lr2list 1 p
		in
		...
		dbg_print "check_noninput_dependence 1";
		(*force the infered_assertion_array_lst_old to be invalid*)
		if((List.length infered_assertion_array_lst_old_nonloop)!=0) then begin
			let orass_sim = List.map (fun x -> simplify_withBDD x ddM) infered_assertion_array_lst_old_nonloop
			in
			self#force_assertion (or_assertion orass_sim);
		end
		;
		List.iter (fun x -> self#force_assertion (invert_assertion x)) infered_assertion_array_lst_old_loop;
		dbg_print "check_noninput_dependence 2";
		
		self#set_lock_multiple;
		
		(*check the output dependent on state and config*)
		let dff_idxpairlst=List.filter (isdff_name_index) name_index_lst
		in
		let dff_curlst=List.map (fun x -> x+final_index_oneinst*p) (List.map (fun x -> match x with (_,(y,_)) -> y) dff_idxpairlst)
		and dff_nxtlst=List.map (fun x -> x+final_index_oneinst*p) (List.map (fun x -> match x with (_,(_,y)) -> y) dff_idxpairlst)
		and bv_outstrlist_p=List.map (fun x -> x+final_index_oneinst*p) bv_outstrlist
		in
		self#proc_dep_vec (bv_outstrlist_p@dff_nxtlst) (dff_curlst@bv_non_proctocol_input_list)
	end

	
	method proc_dep_vec xlst deplst = begin
		self#set_unlock_multiple;
		
		let newmaxidx=get_largest_varindex_inclslst clause_list_multiple
		in 
		let shiftedcls=shiftclslst clause_list_multiple deplst newmaxidx
		and shifted_xlst=List.map (fun x -> x+newmaxidx) xlst
		in 
		let neqv_clslst = self#encode_INEV xlst shifted_xlst
		in
		let satres=dump_sat_withclear (neqv_clslst@(clause_list_multiple@shiftedcls))
		in begin
			match satres with 
			UNSATISFIABLE -> true
			| _ -> begin
	...
	method check_p_r_l (p:int) (r:int) (l:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI = begin
		(*sent two empty list to findLR_checkO_nonloop such that it can discover a whole new set to replace the current one*)
		let (result_p,infered_assertion_array_lst_new_nonloop)=self#findLR_checkO_nonloop p l r infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI 
		in begin
			...
			if (result_p==UNSATISFIABLE) then begin
				...
				let (result_p_loop,infered_assertion_array_lst_new_loop)=self#findLR_checkO_loop_wrap p l r infered_assertion_array_lst_new_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI(*in this case I dont need to encode common again*)
				in begin
	...
	method findLR_checkO_nonloop_noinfer (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop =begin
		Printf.printf "  findLR_checkO_nonloop_noinfer : p %d l %d r %d nonloop %d\n" p l r (List.length infered_assertion_array_lst_old_nonloop) ;
		flush stdout;
		let target = self#findLR_common p l r  [] infered_assertion_array_lst_old_nonloop (*actually we need to rule out these nonloop assertion*)
		in 
		dump_sat_withclear (([target],"target")::clause_list_multiple)
	end

	method findLR_checkO_nonloop (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI =begin
		Printf.printf "  findLR_checkO_nonloop : p %d l %d r %d nonloop %d loop %d\n" p l r (List.length infered_assertion_array_lst_old_nonloop) (List.length infered_assertion_array_lst_old_loop);
		flush stdout;
		let target = self#construct_nonloop_bitIs p l r  infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI (*TODO: dont forget to shift the variables*)
		in begin
			(*dump to sat*)
			let res=dump_sat_withclear (([target],"target")::clause_list_multiple)
			in begin
				if(res==UNSATISFIABLE) then (res,infered_assertion_array_lst_old_nonloop) (*it is ok already*)
				else begin
					let listUniqBitI_shifted=List.map (fun x -> x+(p+l)*final_index_oneinst) listUniqBitI in
					let (res1,itplst)=allsat_interp clause_list_multiple target listUniqBitI_shifted (self#construct_varlst2assumption p l r listUniqBitI) ddM in
...
(*	method check_loop_bitI (p:int) (l:int) (r:int) bitI  =begin
		dbg_print "check_loop_bitI before  construct_nonloop_bitIs";
		let target=self#construct_nonloop_bitIs p l r [] [] [] [bitI] in begin
			self#set_unlock_multiple ;
			let target_loop_p = self#double_loop p l r 0 p
			and target_loop_l = self#double_loop p l r p (p+l)
			and target_loop_r = self#double_loop p l r (p+l+1) (p+l+1+r-1)
			...
			in begin
				...
				self#append_clause_list_multiple cls_red_and_all ;
				self#set_lock_multiple ;
				dbg_print "check_loop_bitI before  dump_sat_withclear";
				let res=dump_sat_withclear (([target_all],"target")::clause_list_multiple) in begin
					dbg_print "check_loop_bitI after  dump_sat_withclear";
					res
				end
			end
		end
...
*)
	method check_nonloop_bitI (p:int) (l:int) (r:int) bitI  =begin
		dbg_print "check_nonloop_bitI before construct_nonloop_bitIs";
		let target=self#construct_nonloop_bitIs p l r [] [] [] [bitI] in begin
			let res=dump_sat_withclear (([target],"target")::clause_list_multiple) in begin
				dbg_print "check_nonloop_bitI after dump_sat_withclear";
				res
			end
		end
	end
	...
	method construct_nonloop_1copy (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop =begin
		...
		*)
		let (clause_list_multiple_aux,last_index_aux) = (self#gen_multiple_instance_step2 (p+l+1+r))
		in begin
			self#set_clause_list_multiple clause_list_multiple_aux;
			self#set_last_index last_index_aux;
			()
		...
		if (List.length (List.filter (self#is_assertion) name_index_lst)) > 0 then
			self#append_clause_list_multiple (self#try_assertion 0 (p+l+1+r-1))
		else 
			dbg_print "warning : not found assertion_shengyushen"
		;
		
		(*connect the first p+1+r instance*)
		let clslst=(self#connect_multiple_instance_step3 0 (p+l+r)) in begin
			dbg_print "construct_nonloop_bitIs 4.5";
			self#append_clause_list_multiple  clslst ;
		end;
		(*check_clslst_maxidx clause_list_multiple last_index;*)
		
		(*TODO : shift the assertion to proper cycle*)
		let ass_nonloop=List.map (fun x -> shiftAssertion x ((p+l)*final_index_oneinst)) infered_assertion_array_lst_old_nonloop 
		and ass_loop   =List.map (fun x -> shiftAssertion x ((p+l)*final_index_oneinst)) infered_assertion_array_lst_old_loop   in begin
			(*force the infered_assertion_array_lst_old to be invalid*)
			...
			;
			(*dbg_print "findLR_common 2";*)
			List.iter (fun x -> self#force_assertion (invert_assertion x)) ass_loop;
		end
	end

	method construct_nonloop (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop =begin
		self#set_unlock_multiple;
		...
		*)
		...
		;
		dbg_print "construct_nonloop_bitIs 3";
		...
		dbg_print "construct_nonloop_bitIs 4";
		(*connect the first p+1+r instance*)
		let clslst=(self#connect_multiple_instance_step3 0 (p+l+r)) in begin
			dbg_print "construct_nonloop_bitIs 4.5";
			self#append_clause_list_multiple  clslst ;
		...
		dbg_print "construct_nonloop_bitIs 5";
		(*connect the secnod p+1+r instance*)
		let clslst2=(self#connect_multiple_instance_step3 (p+l+1+r) ((p+l+1+r)*2-1)) in begin
			dbg_print "construct_nonloop_bitIs 5.5";
		...
		in begin
		dbg_print "construct_nonloop_bitIs 10";
	...
	method findLR_common (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop =begin
		self#set_unlock_multiple;
		...
		*)
		...
		(*force the npi to be the same
		in inferring application, these will be used to force inferred assertion on all cycles
		but in other cases, these will only not be used because try_assertion have already force npi assertion for every cycle*)
		let npi_lst=bv_non_proctocol_input_list
	...
	method findLR_checkO_loop_wrap (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI=begin
	...
	method reduce_p (p:int) (l:int) (d:int) infered_assertion_array_lst_old=begin
		if(p==0) then p
			...
			let res=self#findLR_checkO_nonloop_noinfer (p-1) l d infered_assertion_array_lst_old
			in begin
	...
	method reduce_d (p:int) (l:int) (d:int) infered_assertion_array_lst_old=begin
		if(d==0) then d 
			...
			let res=self#findLR_checkO_nonloop_noinfer p l (d-1) infered_assertion_array_lst_old
			in begin
	...
	method rmred_check (p:int) (l:int) (d:int) infered_assertion_array_lst_old=begin
		...
		*)
		...
		self#set_lock_multiple ;
		(*dump to sat*)
		dump_sat_withclear clause_list_multiple
	end

	method gen_dec_and_ia  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_new_nonloop start_time_p2=begin
		assert(lock_oneinst==true);
		assert(lock_multiple==true);
		(* this is functional depedency test *)
		dbg_print "gen_dec_and_ia 1";
		...
		begin
			...
			dbg_print "gen_dec_and_ia 2";
			fd_res := self#fd_test_I p1 l1 d1 (!clist);
			dbg_print "gen_dec_and_ia 3";
			assert(lock_oneinst==true);
			assert(lock_multiple==true);
			...
			while ((!fd_res_SAT)!=UNSATISFIABLE) do
				...
				dbg_print "gen_dec_and_ia 4";
				fd_res := self#fd_test_I p1 l1 d1 (!clist) ;
				dbg_print "gen_dec_and_ia 5";
				fd_res_SAT := fst (!fd_res) ; 
				bv_C := snd (!fd_res) ; 
			done
			;
			...
			let prt_idx idddx= begin
				if(idddx>0) then 
					dbg_print (sprintf " %s %d" (self#idx2name idddx) idddx)
				else 
					dbg_print (sprintf " !%s %d" (self#idx2name (-idddx)) idddx)
			end
			in
			...
			dbg_print "gen_dec_and_ia 6";
			let infass= begin
				if (List.length infered_assertion_array_lst_new_nonloop)!=0 then 
					simplify_withBDD (invert_assertion (or_assertion infered_assertion_array_lst_new_nonloop)) ddM
				else 
					Array.make 1 TiterpCircuit_true
			end
			in
			let ialst=List.map (self#gen_IA_I p1 l1 d1 infered_assertion_array_lst_new_nonloop) (!clist)
			in
			...
			in begin
				...
				dbg_print "gen_dec_and_ia 7";
				assert(lock_oneinst==true);
				assert(lock_multiple==true);
				assert(((List.length (!clist))-1)==((List.length ialst)-1));
				(*generate these functions iteratively*)
				let rng_clist= lr2list 0 ((List.length ialst)-1)
				in
				let rng_clist_cmd=List.combine rng_clist ialst
				in 
				List.iter (fun x -> match x with (i,ia) -> self#gen_dec_function p1 l1 d1 i ia) rng_clist_cmd 
				;

			end
			;
			
			dbg_print "gen_dec_and_ia 8";
	...
	method gen_dec_function (p1:int) (l1:int) (d1:int) dec_counter ia = begin
		(*let prt_idx idddx= begin
			if(idddx>0) then 
				dbg_print (sprintf "haha %s %d" (self#idx2name idddx) idddx)
			else 
				dbg_print (sprintf "haha !%s %d" (self#idx2name (-idddx)) idddx)
		end
		in
		...
		let dd=(get_largest_varindex_inclslst clause_list_multiple)
		in begin
			dbg_print (sprintf "dd %d" dd);
			self#set_last_index dd;
		end
		;
		(*force the infered_assertion_array_lst_old to be*)
		self#force_assertion ia;
	...
	method gen_IA_I (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old c = begin
      ...
      method fd_test_I (p1:int) (l1:int) (d1:int) clist = begin
		printf "fd_test \n";
		flush stdout;
		...
		in begin
			...
			let satres=dump_sat clause_list_multiple (*must NOT clear because we still need the assigment*)
			in
			(satres,bv_C)
		end
	...
	method gen_relation_pld  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old =begin
		...
		in begin
			...
			;
			check_clslst_maxidx clause_list_multiple last_index;
			printf "last_index %d bd %d final_index_oneinst %d p1 %d d1 %d l1 %d\n" last_index bd final_index_oneinst p1 d1 l1;
			dbg_print "gen_relation_pld 4";
			(*force the npi to be the same*)
			let npi_lst=bv_non_proctocol_input_list
			...
			printf "last_index %d\n" last_index;
			dbg_print "gen_relation_pld 5";
			check_clslst_maxidx clause_list_multiple last_index;
			dbg_print "gen_relation_pld 55";
			(*force the infered_assertion_array_lst_old to be invalid*)
			List.iter (fun x -> self#force_assertion (invert_assertion x)) infered_assertion_array_lst_old ;
			
			dbg_print "gen_relation_pld 6";
			self#set_lock_multiple ;
		end
	end
