...
*)
let rec characterization_interp_AB assumps clslst_1A clslst_0B max_index oidxlst = begin 
		...
		begin
			...
			in 
			let mappedAssumps=List.map (Dumpsat.proc_int )  assumps in
			let trace=begin
				...
				(*solve it*)
				 ...
				 match MiniSAT.solve_with_assumption (mappedAssumps ) with
				 UNSAT -> begin
...
and characterization_interp clslst_noass assumption_lst oidxlst iidx = begin
		...
		let clslst_1A = clslst 
		and clslst_0B = clslst_shift 
		in
		characterization_interp_AB [iidx;(-iidx-max_index)] clslst_1A clslst_0B max_index oidxlst
end
...
begin
	...
	in 
	begin
		dbg_print "exiting allsat_interp";
		x
	end
end
