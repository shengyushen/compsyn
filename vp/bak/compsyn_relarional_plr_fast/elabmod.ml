...
object (self)
	...
	*)
	...
	method compsyn (*(bound_dl:int)*) (instrlist1:string list) (outstrlist1:string list) = begin
		...
		(*simplifying valid(f)*)
		(*****************************************)
			dbg_print "exiting inferPredicateUniq";
	...
	method generateDecoderVerilog prefix left right idx2decfunList = begin
		...
		in begin
			...
			;
			(*printing the wire definition first*)
			let prtFunction idx2decfun = begin
				match idx2decfun with
				(idx,decfun) -> begin
					let size=Array.length decfun in begin
						for i= 1 to (size -1) do
							fprintf decoderVerilog "wire w_%d_%d ;\n" idx i;
						done;
					end
				end
			end in
	...
	method inferPredicateUniq listUniqBitI listNonuniqBitI maxN = begin
		...
		in begin
			while ( (!res) != RES_UNIQ ) do
				...
				xxx := self#inferSATFormula_plr (!b) (!b) (!b) (!infered_assertion_array_lst_old_nonloop) (!infered_assertion_array_lst_old_loop) listUniqBitI listNonuniqBitI ;
	...
	method inferSATFormula_plr (p:int) (r:int) (l:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop listUniqBitI listNonuniqBitI = begin
		...
		in begin
			dbg_print "after inferSATFormula_plr_nonloop";
	...
	method construct_nonloop_1copy (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		...
		dbg_print "construct_nonloop_1copy";
		self#set_unlock_multiple;
	...
	method construct_nonloop (p:int) (l:int) (r:int) infered_assertion_array_lst_old_nonloop infered_assertion_array_lst_old_loop = begin
		Printf.printf "construct_nonloop p %d l %d r %d\n" p l r;
		self#set_unlock_multiple;
