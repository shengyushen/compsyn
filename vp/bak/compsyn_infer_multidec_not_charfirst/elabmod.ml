...
object (self)
	...
	*)
	...
	method proc_getvector_value name = begin
		...
		in
		let (l,_) = self#rng2lr rng
		in 
		let bitlist=List.map (fun x -> baseidx+x-l) (self#rng2list rng)
		in
		let bit_value_list=List.map (self#get_assignment) bitlist
		in
	...
	method dump_sat = begin
		MiniSAT.reset ();
		let max_index=get_largest_varindex_inclslst clause_list_multiple
		in begin
			...
			in
			List.iter proc_cls  clause_list_multiple
			;
	...
	method check_p_r_l (p:int) (r:int) (l:int) infered_assertion_array_lst_old = begin
		...
		in begin
			if (result_p==UNSATISFIABLE) then begin
				...
				self#check_assertion_satisfiable infered_assertion_array_lst_old ;
				...
				in begin
					...
					(*I need to check the wall case*)
					
					let (result_p_wall,infered_assertion_array_lst_wall)=self#findLR_checkO_wall_wrap p l r infered_assertion_array_lst_new 
					in begin
						assert (result_p_wall==UNSATISFIABLE) ;
						(RES_UNK,p,l,r,infered_assertion_array_lst_wall)
					end
		...
		in begin
			while ( (!res) != RES_UNIQ ) do
				...
				(res1,p1,l1,r1,infered_assertion_array_lst_old1) -> begin
					...
					l := l1 ;
					r := r1 ;
					infered_assertion_array_lst_old := infered_assertion_array_lst_old1 ;
				end
			done
			;
	...
	method findLR_common (p:int) (l:int) (r:int) infered_assertion_array_lst_old =begin
		...
		*)
		...
		if (List.length (List.filter (self#is_assertion) name_index_lst)) > 0 then
			clause_list_multiple <- (self#try_assertion 0 ((p+l+1+r)*2-1)) @ clause_list_multiple
		else 
			print_endline "warning : not found assertion_shengyushen"
		;
		
		(*connect the first p+1+r instance*)
		clause_list_multiple <- (self#connect_multiple_instance_step3 0 (p+l+r)) @ clause_list_multiple ;
		
		(*connect the secnod p+1+r instance*)
		clause_list_multiple <- (self#connect_multiple_instance_step3 (p+l+1+r) ((p+l+1+r)*2-1)) @ clause_list_multiple ;
		...
		and npi_cycle_lst = self#lr2list 1 ((p+l+1+r)*2-1)
		in
		let proc_npi_cycle i = begin
			let npi_lst_i = List.map (fun x -> x+(i*final_index_oneinst)) npi_lst
			in
			clause_list_multiple <- (self#encode_EQUV (npi_lst) (npi_lst_i)) @ clause_list_multiple
		end
		in
		...
		List.iter (fun x -> match x with (_,y) -> self#force_assertion (self#invert_assertion y)) infered_assertion_array_lst_old ;
		
		(*force the output of p to p+l+1+r-1 to be the same*)
		let ov=bv_outstrlist
		and outputframe_idx= self#lr2list (p) (p+l+1+r-1)
		in
	...
	method or_assertion arr_itpo1 = begin
		if((List.length arr_itpo1)!=0) then begin
			let (newitpoarr,newlst)=self#construct_ITPLST  arr_itpo1
			in 
	...
	method and_assertion arr_itpo1 = begin
		if((List.length arr_itpo1)!=0) then begin
			let (newitpoarr,newlst)=self#construct_ITPLST  arr_itpo1
			in 
			let finalrand=Array.make 1 TiterpCircuit_none
	...
	method invert_assertion arr_itpo1 = begin
	...
	method force_assertion arr_itpo1 = begin
		let topidx=self#encode_assertion arr_itpo1
		in (*this is the function enabler*)
		clause_list_multiple <- ([topidx],"f i") :: clause_list_multiple 
	end
	
	method encode_assertion arr_itpo1 = begin
			...
			in
			let base_index=self#alloc_index size (*index i of arr_itpo correspond to based_index+i*)
			in
			...
			let rec interpObj2str interpObj = begin
				...
				TiterpCircuit_true -> begin
					let newvar_idx = self#alloc_index 1
					in begin
						clause_list_multiple <- ([newvar_idx],"") :: clause_list_multiple ;
						newvar_idx
					end
				end
				...
				| TiterpCircuit_and(interpObjlst) -> begin
					...
					and base_index_subsub=self#alloc_index 1
					in begin
						clause_list_multiple <- (self#encode_Red_AND base_index_subsub objreslst ) @ clause_list_multiple ;
						base_index_subsub
					end
				end
				| TiterpCircuit_or(interpObjlst) -> begin
					...
					and base_index_subsub=self#alloc_index 1
					in begin
						clause_list_multiple <- (self#encode_Red_OR base_index_subsub objreslst ) @ clause_list_multiple ;
						base_index_subsub
					end
				end
				| TiterpCircuit_not(interpObj) -> begin
					...
					and base_index_subsub=self#alloc_index 1
					in begin
						clause_list_multiple <- (self#encode_NEG base_index_subsub objres ) @ clause_list_multiple ;
						base_index_subsub
					end
				end
			...
			and prt_trace_withInterp num iter_res  = begin
				...
				| _ -> begin
					...
					in
					clause_list_multiple <- (self#encode_EQU str_of_itpo (arridx2varidx num)) @ clause_list_multiple 
				end
			end
			in begin
				prt_trace_withInterp (size-1) arr_itpo.(size-1) ;
				(arridx2varidx (size-1))
			end
	end
	...
	method check_itpo arr_itpo = begin
			let size=Array.length arr_itpo
			in
			let rec interpObj2str interpObj = begin
				match interpObj with
	...
	method construct_instance_loop (p:int) (l:int) (r:int) target_all_old =begin
		...
		in begin
			...
			clause_list_multiple <- cls_red_and_all@clause_list_multiple ;
			target_all
		end
	end
	
	method check_assertion_satisfiable infered_assertion_array_lst_old=begin
				printf "  check_assertion_satisfiable \n";
				...
				in begin
					...
					| SATISFIABLE -> begin
					  if ((List.length infered_assertion_array_lst_old) > 0 ) then begin
						let npi_index_value_list = List.map (self#proc_getvector_value) non_proctocol_input_list
						in
						let npi_name_value = List.combine non_proctocol_input_list npi_index_value_list
						in begin
							printf "  SOLUTION : " ;
							flush stdout ;
							List.iter (fun x -> match x with (name,intvalue) -> printf " %s %d " name intvalue ) npi_name_value ; 
							printf "\n" ;
							()
						end
					  end
	...
	method construct_wallcase (p:int) (l:int) (r:int) infered_assertion_array_lst_old= begin
			let target = self#findLR_common p l r infered_assertion_array_lst_old
			in begin
				(*wall constraints*)
				let dff_idxpair=List.filter (self#isdff_name_index) name_index_lst
				in
				let dff_current_lst=List.map (fun x-> match x with (_,(c,_))-> c) dff_idxpair
				and dff_next_lst=   List.map (fun x-> match x with (_,(_,n))-> n) dff_idxpair
				in
				let dff_current_lst_p=       List.map (fun x -> x+p*final_index_oneinst)               dff_current_lst
				and dff_current_lst_p_pl1r=  List.map (fun x -> x+(p+p+l+1+r)*final_index_oneinst)     dff_current_lst
				and dff_current_lst_plr=     List.map (fun x -> x+(p+l+r)*final_index_oneinst)         dff_next_lst
				and dff_current_lst_plr_pl1r=List.map (fun x -> x+(p+l+r+p+l+1+r)*final_index_oneinst) dff_next_lst
				in
				let (left_target,left_oclslist)=self#encode_EQUV_res dff_current_lst_p dff_current_lst_p_pl1r 
				and (right_target,right_oclslist)=self#encode_EQUV_res dff_current_lst_plr dff_current_lst_plr_pl1r 
				in begin
					clause_list_multiple <- left_oclslist@right_oclslist@clause_list_multiple ;
					
					let new_target=self#alloc_index 1
					in begin
						let andres_clslst=self#encode_Red_AND new_target [left_target;right_target;target]
						in begin
							clause_list_multiple <- andres_clslst@clause_list_multiple ;
							new_target
						end
					end
				end
	...
	method findLR_checkO_wall_wrap (p:int) (l:int) (r:int) infered_assertion_array_lst_old=begin
		Printf.printf "  findLR_checkO_wall_wrap : p %d l %d r %d ass len %d\n" p l r (List.length infered_assertion_array_lst_old);
		flush stdout;
		if(p<2 || l<2 || r<2) then (UNSATISFIABLE,infered_assertion_array_lst_old)
		else begin
			let target_all = self#construct_wallcase p l r infered_assertion_array_lst_old
			in
			let res= ref SATISFIABLE
			and infered_assertion_array_lst_new = ref []
			and xxx = ref (SATISFIABLE,[])
			in begin
				infered_assertion_array_lst_new := infered_assertion_array_lst_old;
				
				while ((!res)!=UNSATISFIABLE) do
					xxx := self#check_and_infer p l r target_all (!infered_assertion_array_lst_new);
					match (!xxx) with
					(res1,infered_assertion_array_lst_new1) -> begin
						res := res1;
						infered_assertion_array_lst_new := infered_assertion_array_lst_new1;
					end
				done
				;
	...
	method check_and_infer (p:int) (l:int) (r:int) target_all infered_assertion_array_lst_old=begin
		Printf.printf "  check_and_infer : p %d l %d r %d ass len %d\n" p l r (List.length infered_assertion_array_lst_old);
		flush stdout;
				(*save the encoded instance*)
				clause_list_multiple_saved <- clause_list_multiple ;
				last_index_saved <- last_index ;
					
				clause_list_multiple <- ([target_all],"target_all")::clause_list_multiple
				;
				...
				in begin
					match satres with
					UNSATISFIABLE -> begin
						(*check if it is inferable*)
						Printf.printf "  no result in  check_and_infer\n";
						flush stdout;
						(satres,infered_assertion_array_lst_old)
					end
					| _ -> begin
						Printf.printf "  found result in  check_and_infer\n";
						flush stdout;
						(*construct the list of all S0 and protocol input*)
						let varlst2assumption = self#construct_varlst2assumption p l r
						in (*get all their value, and construct the assumption list*)
						let all_index_value_list = List.map (self#get_assignment) varlst2assumption
						in begin
							(*construct the instance by copy back above created instance*)
							clause_list_multiple <- clause_list_multiple_saved ;
							last_index <- last_index_saved ;
							
							let target_all_again = target_all
							and npi_lst=bv_non_proctocol_input_list
							in (*generate the circuit*)
							let (iidxxx,new_assertion_preBDD) = characterization_interp clause_list_multiple all_index_value_list npi_lst target_all_again
							in
							let new_assertion = self#simplify_withBDD new_assertion_preBDD 
							in begin
								assert (target_all_again==target_all) ;
								
								(*print the inferred invalid configure formula*)
								printf "\n++++++++++invalid configure formula++++++++++\n";
								self#print_itpo new_assertion;
								printf "+++++++++++++++++++++++++++++++++++++++++++++++\n";
								
								(*increamental encoding the new_assertion*)
								self#force_assertion (self#invert_assertion new_assertion);
								(*check that all var referenced in new_assertion are npi*)
								self#check_assertion new_assertion npi_lst ;
								
								let infered_assertion_array_lst_new = ((iidxxx,new_assertion)::infered_assertion_array_lst_old) 
								in
								(satres,infered_assertion_array_lst_new)
							end
						end
					end
				end
	...
	method simplify_withBDD_withassumption itpcircuit assumptionlist= begin
		let arr_itpo=self#simplify_withBDD itpcircuit
		in begin
				...
				self#simplify_withBDD arr_itpo
		end
	end
	
	method simplify_withBDD arr_itpo = begin
					...
					let rec find_nonpos itpo_elem = begin
						...
						| TiterpCircuit_and(itpolst) -> begin
							if ((List.length itpolst)==0) then begin
								...
								in 
								T_bddssy_node(self#bdd_andlst (List.map (self#return_bdd) bddlst))
							end
						end
						| TiterpCircuit_or(itpolst) -> begin
							if ((List.length itpolst)==0) then begin
								...
								let bddlst=List.map find_nonpos itpolst
								in 
								T_bddssy_node(self#bdd_orlst (List.map (self#return_bdd) bddlst))
							end
						end
						| TiterpCircuit_not(itpo) -> begin
							T_bddssy_node(CaddieBdd.bddNot (self#return_bdd (find_nonpos itpo)))
						end
						| TiterpCircuit_none -> begin
							assert false ;
						end
					...
					in begin
						...
						| TiterpCircuit_and(itpolst) -> begin
							if ((List.length itpolst)==0) then begin
								...
								let bddlst=List.map find_nonpos itpolst
								in 
								let res=self#bdd_andlst (List.map (self#return_bdd) bddlst)
								in begin
									Array.set bddarray pos (T_bddssy_node(res));
									(T_bddssy_node(res))
								end
						...
						| TiterpCircuit_or(itpolst) -> begin
							if ((List.length itpolst)==0) then begin
								...
								let bddlst=List.map find_nonpos itpolst
								in 
								let res=self#bdd_orlst (List.map (self#return_bdd) bddlst)
								in begin
									Array.set bddarray pos (T_bddssy_node(res));
									(T_bddssy_node(res))
								end
						...
						| TiterpCircuit_not(itpo) -> begin
							let res=T_bddssy_node(CaddieBdd.bddNot (self#return_bdd (find_nonpos itpo)))
							in begin
								assert ( res!=T_bddssy_non );
								Array.set bddarray pos res;
								res
	...
	method return_bdd wrap_bdd = begin
		match wrap_bdd with
		T_bddssy_non -> assert false
		| T_bddssy_node(bddfinal) -> bddfinal
	end
	
	method bdd_andlst bddlst = begin
	...
	method check_assertion arr_itpo npi_lst = begin
	...
	method get_assignment idx = begin
		assert (idx > 0) ;
	...
	method get_assignment_lit idx = begin
		assert (idx > 0) ;
		let v=MiniSAT.value_of idx
		in begin
			match v with
	...
	method construct_varlst2assumption (p:int) (l:int) (r:int) = begin
		...
		let ov=bv_instrlist
		and cycle_list=self#lr2list 0 (((p+l+1+r)*2)-1)
		in
	...
	method findLR_checkO (p:int) (l:int) (r:int) infered_assertion_array_lst_old=begin
		...
		in begin
			...
			clause_list_multiple <- ([target],"target")::clause_list_multiple
		end
		;

		(*dump to sat*)
		let res=self#dump_sat 
		in
	...
	method rmred_check (p:int) (l:int) (d:int) infered_assertion_array_lst_old=begin
		...
		*)
		...
		(*constrain the assertion wire*)
		if (List.length (List.filter (self#is_assertion) name_index_lst)) > 0 then
			clause_list_multiple <- (self#try_assertion 0 ((p+1+d)*2-1)) @ clause_list_multiple
		else 
			print_endline "warning : not found assertion_shengyushen"
		;
		
		(*connect the first p+d instance*)
		clause_list_multiple <- (self#connect_multiple_instance_step3 0 (p+d)) @ clause_list_multiple ;
		
		(*connect the secnod p+d instance*)
		clause_list_multiple <- (self#connect_multiple_instance_step3 (p+1+d) ((p+1+d)*2-1)) @ clause_list_multiple ;
		
		(*force the output to be the same*)
		let ov=bv_outstrlist
		and outputframe_idx= self#lr2list (p+1+d-1-l+1) (p+1+d-1)
		in
		let ov_b_1 = List.concat (List.map (fun frmidx -> (List.map (fun x -> x+ frmidx*final_index_oneinst) ov) ) outputframe_idx)
		in
		...
		(*force the npi to be the same*)
		let npi_lst=bv_non_proctocol_input_list
		and npi_cycle_lst = self#lr2list 1 ((p+1+d)*2-1)
		in
		let proc_npi_cycle i = begin
			let npi_lst_i = List.map (fun x -> x+(i*final_index_oneinst)) npi_lst
			in
	...
	method resolve cls1 cls2 piv = begin
		let cls1_nopiv=List.filter (fun x -> (abs x)!=piv ) cls1
		and cls2_nopiv=List.filter (fun x -> (abs x)!=piv ) cls2
		in
		let cls1_nopiv_nodup=List.filter (fun x -> if (List.mem x cls2_nopiv) then false else true) cls1_nopiv
		in
		cls1_nopiv_nodup @ cls2_nopiv
	end
	...
	method gen_characterizer_wrap_new  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old =begin
		...
		let outputframe_idx= self#lr2list (p1+1+d1-1-l1+1) (p1+1+d1-1)
		in
		...
		in 
		let lst_idx_f_new = List.map (fun x -> match x with (idx1,(idx2,f)) -> begin assert (idx1==idx2); (idx2,f)end) lst_idx_idx_f
		in begin
			(*dont cover the already encodered relation
			it will be used to constraint the domain of bv_O*)
			(*clause_list_multiple <- [];
			last_index <- 3;*)
			...
			let proc_idx_f idx_f = begin
				match idx_f with
				(idx,f) -> begin
					let topidx=self#encode_assertion f
					in
					clause_list_multiple <- (self#encode_EQU idx topidx)  @ clause_list_multiple 
				end
			end
			in
			...
			self#gen_characterizer_wrap  p1 l1 d1 infered_assertion_array_lst_old
		end
	end

	method gen_characterizer_wrap  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old =begin
		...
		begin
			...
			fd_res := self#fd_test_I p1 l1 d1 infered_assertion_array_lst_old (!clist);
			...
			while ((!fd_res_SAT)!=UNSATISFIABLE) do
				...
				in
				let c_litlst_new1 = List.map (self#get_assignment_lit) (!bv_C) 
				in
			...
			in
			let iarange=self#lr2list 0 ((List.length iac_list_new)-1)
			in
			...
			let rng_clist= self#lr2list 0 ((List.length (!clist))-1)
			in
...
(*	method uniquify clist = begin
		match clist with
		[] -> []
		| c::ctl -> begin
			let new_ctl=self#uniquify ctl
			in begin
				let pred_equlst nc = begin
					assert((List.length nc)==(List.length c));
					let ccnc=List.combine c nc
					in 
					List.for_all (fun x -> match x with (c1,c2) -> (c1==c2)) ccnc
				end
				in begin
					if(List.exists (pred_equlst) new_ctl) then new_ctl else (c::new_ctl)
				end
			end
		end
	end*)
	...
	method fd_test_gen_char (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old cb = begin
		...
		(i,c) -> begin
			...
			in begin
				...
				let new_assertion_c = (0,new_arr_itpo)
				in(*todotodo this is currently trying*)
				(*self#gen_characterizer  p1 l1 d1 infered_assertion_array_lst_old i*)
				self#gen_characterizer  p1 l1 d1 [new_assertion_c] i
			end
		end
	end
	...
	method construct_ITPLST r_asslst = begin
		match r_asslst with
		[] -> begin
			assert false
		end
		...
		| hd::tl -> begin
			let xx=self#construct_ITPLST tl
			in begin
				match xx with
	...
	method fd_test_I (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old clist = begin
		printf "fd_test \n";
		flush stdout;
		...
		and bv_I_rangelist = if((List.length clist)==0) then [] else self#lr2list 0 ((List.length clist)-1)
		in
	...
	method encode_R i c dontshift_lst = begin
		clause_list_multiple <- (shiftclslst clause_list_multiple_f dontshift_lst (i*last_index_f)) @ clause_list_multiple ;
		last_index <- (i+1)*last_index_f
	end
	...
	method gen_relation_pld  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old =begin
		...
		in begin
			...
			(*force the npi to be the same*)
			let npi_lst=bv_non_proctocol_input_list
			and npi_cycle_lst = self#lr2list 1 (bd-1)
			in
			let proc_npi_cycle i = begin
				let npi_lst_i = List.map (fun x -> x+(i*final_index_oneinst)) npi_lst
				in
			...
			in begin
			  ...
			  if(pres==RES_UNIQ) then begin
				...
				in begin
					...
					let infered_assertion_array_lst_old_loop1=List.map (fun x -> match x with (_,y) -> y) infered_assertion_array_lst_new
					in begin
					if (List.length infered_assertion_array_lst_old_loop1)!=0 then begin
						let infass=self#simplify_withBDD (self#invert_assertion (self#or_assertion infered_assertion_array_lst_old_loop1))
						in begin
						...
						printf "the current remained assertion :\n";
						printf "++++++++++++++++++++++++++\n True";
						printf "\n++++++++++++++++++++++++++\n";
					end
					end
					...
					(*if((List.length infered_assertion_array_lst_new )>0) then begin
						printf "the infered assertion :\n";
						printf "++++++++++++++++++++++++++\n";
						self#print_neg_assertion (List.hd infered_assertion_array_lst_new);
						printf "\n" ;
						List.iter (fun x -> begin printf " & " ;self#print_neg_assertion x end) (List.tl infered_assertion_array_lst_new);
						printf "++++++++++++++++++++++++++\n";
					end 
					;*)
			  	...
			  	(*let end_time = Unix.gettimeofday ()
				in
				Printf.fprintf stdout "\n TIME : check_compsyn  %f \n" (end_time -. start_time_check_comp)
				;

				(SATISFIABLE,0,0,0,0)*)
				(*above should not happen while infering assertion*)
				assert false 
			  end
			end
		end
					...
					if (rli_real_cur==new_current_idx) && (new_current_idx-new_next_index==final_index_oneinst*(dst_i-src_i)-(abs(rlr-rli)+1)) && (rli_real_cur-final_index_oneinst*(dst_i-src_i)+abs(rlr-rli)+1==rli_real_nxt) then begin
						...
						let rec connect_1bitdff idxi idxr = begin
							if idxi>idxr then []
								...
								in
								let newmapped_commented= List.map (self#setcomment (sprintf "connecting dff %s from instance %d to %d" nm src_i dst_i)) newmapped 
								in
	...
	method gen_multiple_instance_step2 num = begin
		let lrlst=self#lr2list 0 (num-1)
		in
	...
	method encode_EQUV_res l1 l2 = begin
		if (List.length l1) == (List.length l2) then  begin
			...
			in
			let new_var_lst = self#lr2list new_base (new_base+len1-1)
			in
			let ov_pair=List.combine l1 l2
			in
	...
	*)
	...
	method isdff_name_index name_index = begin
