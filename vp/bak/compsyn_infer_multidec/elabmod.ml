	...
	*)
	...
	method findLR_common (p:int) (l:int) (r:int) infered_assertion_array_lst_old =begin
		...
		*)
		...
		List.iter (fun x -> match x with (_,y) -> self#falsify_infered_assertion y) infered_assertion_array_lst_old ;
	...
	method falsify_infered_assertion arr_itpo1 = begin
(*		printf "    new_assertion size %d\n" (Array.length (snd inidx_function));
		printf "    before man size %d\n" (List.length clause_list_multiple);
		flush stdout;*)
			...
			in
			let arridx2varidx i =  base_index + i
			...
			in begin
				...
				(*this is the function disabler*)
				clause_list_multiple <- ([-(arridx2varidx (size-1))],"f i") :: clause_list_multiple ;
				
				(*let end_time = Unix.gettimeofday ()
				in
				printf "    after man size %d take time %f\n" (List.length clause_list_multiple) (end_time -. start_time)
				;
	...
	(*the only difference between force_infered_assertion and falsify_infered_assertion 
	is that the former one dont disable the function*)
	method force_infered_assertion inidx_function = begin
		(*printf "    new_assertion size %d\n" (Array.length (snd inidx_function));
		printf "    before man size %d\n" (List.length clause_list_multiple);
		flush stdout;*)
		match inidx_function with
		(_,arr_itpo1) -> begin
			let start_time = Unix.gettimeofday ()
			in
			let arr_itpo=Array.copy arr_itpo1
	...
	(*method print_neg_assertion inidx_function = begin
		match inidx_function with
		(_,arr_itpo) -> begin
			self#print_neg_itpo arr_itpo
		end
	end
	*)
	method print_neg_itpo arr_itpo = begin
			let size=Array.length arr_itpo
			in
			let rec interpObj2str interpObj = begin
				match interpObj with
		...
		if(p<2 || l<2 || r<2) then (UNSATISFIABLE,infered_assertion_array_lst_old)
			...
			in begin
				...
				while ((!res)!=UNSATISFIABLE) do
					xxx := self#findLR_checkO_loop p l r target_all (!infered_assertion_array_lst_new);
					match (!xxx) with
	...
	method findLR_checkO_loop (p:int) (l:int) (r:int) target_all infered_assertion_array_lst_old=begin
		Printf.printf "  findLR_checkO_loop : p %d l %d r %d ass len %d\n" p l r (List.length infered_assertion_array_lst_old);
		flush stdout;
				...
				in begin
					...
					UNSATISFIABLE -> begin
						...
						Printf.printf "  no loop in  findLR_checkO_loop\n";
						flush stdout;
						(satres,infered_assertion_array_lst_old)
					end
					| _ -> begin
						Printf.printf "  found loop in  findLR_checkO_loop\n";
						flush stdout;
						...
						in begin
							...
							in begin
								...
								self#falsify_infered_assertion new_assertion;
				...
				in begin
					...
					| T_bddssy_node(bddfinal) -> begin
						...
						in begin
							...
							if ((List.length (!itpolst4bdd))==0) then begin
	...
	method print_inidx_function compsyn_res_verilog inidx_function = begin
		match inidx_function with
		(iidx,arr_itpo) -> begin
			let rec interpObj2str interpObj = begin
				match interpObj with
				...
				| TiterpCircuit_refcls(clsidx) -> begin
					...
					in begin
						match itpo_nxt with
						TiterpCircuit_refcls(_) -> interpObj2str itpo_nxt
						| TiterpCircuit_printed(_) -> sprintf " itp%dc%d " iidx clsidx
						| _ -> begin
							prt_trace_withInterp clsidx itpo_nxt;
							Array.set arr_itpo clsidx (TiterpCircuit_printed(clsidx));
							sprintf " itp%dc%d " iidx clsidx
						end
					end
				end
				| TiterpCircuit_refvar(varidx) -> if varidx > 0 then sprintf " v%d " varidx else sprintf " !v%d " (-varidx)
				| TiterpCircuit_and(interpObjlst) -> begin
					let objreslst=List.map (interpObj2str ) interpObjlst
					in
					String.concat " " ["(" ;(String.concat " & " objreslst);")\n"]
				...
				| TiterpCircuit_printed(clsidx) -> sprintf " itp%dc%d " iidx clsidx
				| _ -> assert false
			end
			and prt_trace_withInterp num iter_res  = begin
				match iter_res with
				...
				| _ -> begin
					let str_of_itpo=interpObj2str  iter_res
					in
					fprintf compsyn_res_verilog "wire itp%dc%d = %s ; \n" iidx  num str_of_itpo
				end
			end
			in 
			let size = Array.length arr_itpo
			in begin
				prt_trace_withInterp (size-1) arr_itpo.(size-1) ;
				fprintf compsyn_res_verilog "assign v%d = itp%dc%d ;\n" iidx iidx (size-1)
			end
		end
	end
	...
	method gen_characterizer_wrap  (p1:int) (l1:int) (d1:int) infered_assertion_array_lst_old =begin
				(*printf "bv_non_proctocol_input_list ";
				List.iter (fun x -> printf " %d" x) bv_non_proctocol_input_list
				;
				printf "\n";*)
		...
		begin
			...
			fd_res := self#fd_test p1 l1 d1 infered_assertion_array_lst_old (!clist);
			...
			List.iter (fun x -> begin List.iter (fun y -> printf " %d" y) x ; printf "\n" end) (!clist) ;*)
			
			(*generate the h function*)
			let ialst=self#gen_IA p1 l1 d1 infered_assertion_array_lst_old (!clist)
			in
			let iarange=self#lr2list 0 ((List.length ialst)-1)
			in
			let idxialst=List.combine iarange ialst
			in
	...
	method encode_C i c = begin
		...
		in begin
			...
			self#force_infered_assertion (0,new_arr_itpo)
		end
	end
			...
			*)
			...
			in begin
			  ...
			  if(pres==RES_UNIQ) then begin
				...
				in begin
					self#gen_characterizer_wrap (p1+l) l1 d1 infered_assertion_array_lst_new;
					...
					if (List.length infered_assertion_array_lst_new)!=0 then begin
						let (newitpoarr,newlst)=self#construct_ITPLST (List.map (fun x -> match x with (_,y) -> y) infered_assertion_array_lst_new)
						in 
						let finalrand=Array.make 1 TiterpCircuit_none
						in begin
							(*i use or because infered_assertion_array_lst_new is reduce and of all neg assertion
							so after simplify_withBDD, there is still another ! at the head of result*)
							Array.set finalrand 0 (TiterpCircuit_or(List.map (fun x -> TiterpCircuit_refcls(x)) newlst));
							let randitpoarr=Array.append newitpoarr finalrand
							in
							let negass=self#simplify_withBDD randitpoarr
							in begin
								printf "the infered assertion :\n";
								printf "++++++++++++++++++++++++++\n";
								self#print_neg_itpo negass;
								printf "\n++++++++++++++++++++++++++\n";
							end 
						end
					end
