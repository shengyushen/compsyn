...
and encode_assertion arr_itpo1 last_index_old = begin
	...
	in begin
		...
		in begin
			...
			in
			let arr_itpo=Array.copy arr_itpo1
			in
...
and check_assertion_satisfiable infered_assertion_array_lst_old last_index=begin
	...
	in
	let npi_res= dump_sat clslstfinal
	in begin
		match npi_res with
		UNSATISFIABLE -> begin
		...
		| SATISFIABLE -> begin
			printf "check_assertion_satisfiable OK ass len %d\n" (List.length infered_assertion_array_lst_old);
		end
	end
end
