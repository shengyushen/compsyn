...
{
//cerr<<"num_free_variables set_var_value "<< num_free_variables()<<endl;
...
{
//	cerr << "num_free_variables11 "<<num_free_variables()<<endl;
    if(dlevel()>0)
...
//    add_outside_clauses();
    ...
    ++_stats.num_decisions;
//	cerr << "num_free_variables== "<<num_free_variables()<<endl;
...
//	    sign = (random() > (RAND_MAX >> 1))? 0: 1;
    ...
    _implication_id = 0;
    queue_implication(s_var, NULL_CLAUSE, dlevel());
//	cerr << "num_free_variables22 "<<num_free_variables()<<endl;
    return true;
}
...
/*	for (unsigned i=1; i< variables().size(); ++i) {
		...
		if (ante >= 0) {
		    ...
		    verify_out << "VAR: " << i 
			       ...
			       << " Lits:";
		    for (unsigned j=0; j< clause(ante).num_lits(); ++j) {
		    	if(clause(ante).literal(j).var_sign()==1)//its value is actually -
				verify_out << " -" << clause(ante).literal(j).var_index();
			else 
				verify_out << " " << clause(ante).literal(j).var_index();
		    }
		    if(clause(ante).status() == ORIGINAL_CL)
				verify_out<<" O "<<endl;
			else if(clause(ante).status() == CONFLICT_CL)
				verify_out<<" C "<<endl;
			else assert(0);
		}
	    }
	}
	verify_out << "CONF: " << clause(_conflicts[0]).id()<<" in grp "<< clause(_conflicts[0]).gflag() << " ==";
	for (unsigned i=0; i< clause(_conflicts[0]).num_lits(); ++i) {
	    if(clause(_conflicts[0]).literal(i).var_sign()==1)
	    	verify_out << " -" << clause(_conflicts[0]).literal(i).var_index();
	    else
	    	verify_out << " " << clause(_conflicts[0]).literal(i).var_index();
	}
	if(clause(_conflicts[0]).status() == ORIGINAL_CL)
		verify_out<<" O "<<endl;
	else if(clause(_conflicts[0]).status() == CONFLICT_CL)
		verify_out<<" C "<<endl;
	else assert(0);*/
#endif
	if(doanalysis_ssy==true) do_ssy_analysis(_conflicts[0]);
	return CONFLICT;
    }
...
int CSolver::solve_ssy_unsat(int clgrp2ext,int ** result2ptr2ptr,int * result_size2ptr,int totry) {
	int res=solve_ssy(clgrp2ext,result2ptr2ptr,result_size2ptr,totry);
	assert(res==UNSATISFIABLE);
	return res;
}

int CSolver::solve_ssy(int clgrp2ext,int ** result2ptr2ptr,int * result_size2ptr,int totry) {
	//normally this is used to extract related clause
	// so we must prevent it from delete clauses
	_params.restart.enable=false;
	doanalysis_ssy=true;
	clgrp_ext=clgrp2ext;
	
	returncls_in1buf.clear();
	return_cls_set.clear();
	visited.clear();
	cl2srccls.clear();
	
	int res=solve();
	assert(res==UNSATISFIABLE || res==SATISFIABLE);
	if(res!=UNSATISFIABLE) {
		return res;
	}
	for(set<ClauseIdx >::iterator itr=return_cls_set.begin();itr!=return_cls_set.end();itr++) {
		CClause cl = clause(*itr);
		//cout<<" in solver ";
		for (unsigned j=0; j< cl.num_lits(); ++j) {
			CLitPoolElement lit = cl.literal(j);
			int litval=lit.s_var();
			returncls_in1buf.push_back(litval);
			//cout<<litval<<" ";
		}
		//cout<<endl;
		returncls_in1buf.push_back(0);
	}
	
	*result2ptr2ptr=&(returncls_in1buf.begin())[0];
	*result_size2ptr=returncls_in1buf.size();
...
/*	verify_out << "CONF: " << clause(shortest).id()<<" in grp "<< clause(shortest).gflag() << " ==";
	for (unsigned i=0; i< clause(shortest).num_lits(); ++i) {
	    if(clause(shortest).literal(i).var_sign()==1)
	    	verify_out << " -" << clause(shortest).literal(i).var_index();
	    else
	    	verify_out << " " << clause(shortest).literal(i).var_index();
	}
	if(clause(shortest).status() == ORIGINAL_CL)
		verify_out<<" O "<<endl;
	else if(clause(shortest).status() == CONFLICT_CL)
		verify_out<<" C "<<endl;
	else assert(0);*/
#endif	
	if(doanalysis_ssy==true) do_ssy_analysis(shortest);
	_conflicts.clear();
	back_track(0);
	return -1;
    }
...
void CSolver::do_ssy_analysis(ClauseIdx cidx) {
	//cout<< "in do ssy " <<cidx<<endl;
	set<int> totry;
	totry.insert(cidx);
	assert(clause(cidx).status() != DELETED_CL);
		
	while(totry.empty()==false) {	
		ClauseIdx tryidx=*(totry.begin());
		totry.erase(totry.begin());
		visited.insert(tryidx);
		//cout<<"totry size "<<totry.size()<<endl;
		assert(clause(tryidx).status() != DELETED_CL);
	   if(clause(tryidx).status() == CONFLICT_CL) {
		//trace back to all its resolvents
		bool found=false;
		for(set<pair<int,set<int> > >::iterator itr=cl2srccls.begin();itr!=cl2srccls.end();itr++) {
			if(((*itr).first)==tryidx) {
				found=true;
				//put them into totry
				for(set<int>::iterator it1=(*itr).second.begin();it1!=(*itr).second.end();it1++) {
					if(visited.find(*it1)==visited.end()) 
						totry.insert(*it1);
				}
				break;
			}
			//cout<<"first "<<((*itr).first)<<" to find " <<tryidx<<endl;
		}
		assert(found==true);
	   } else {
	   	assert(clause(tryidx).status() == ORIGINAL_CL);
		
		if(clause(tryidx).gid(clgrp_ext)==true)
			return_cls_set.insert(tryidx);
		
		for (unsigned i=0; i< clause(tryidx).num_lits(); ++i) {
			int varidx=clause(tryidx).literal(i).var_index();
			int ante = variable(varidx).antecedent();
			if(variable(varidx).value()==UNKNOWN) {
				//cout << "UNKNOWN  varidx "<<varidx<<endl;
				//cout << "UNKNOWN  tryidx "<<tryidx<<endl;
				assert(ante==-1);
			}
			
			if(ante>=0 && ante!=tryidx){//only trace those valid antecedent
				assert(clause(ante).status() != DELETED_CL);
				//cout<<"varidx "<<varidx<<endl;
				//cout<<"ante "<<ante<<endl;
				if(visited.find(ante)==visited.end()) 
					totry.insert(ante);
			} /*else if(ante<0){
				cout<<"neg ante "<<varidx<<" "<<ante<<endl;
				//assert(0);
			}*/
		}
	   }
    ...
    
{
   ...
   if(doanalysis_ssy==true) {
	...
	resolve_cls.insert(_resolvents[i]);
    }
     cl2srccls.insert(pair<int,set<int> >(added_cl,resolve_cls));
    //verify_out << endl;
    _resolvents.clear();
    }
#endif
