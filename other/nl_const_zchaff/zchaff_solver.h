...
{
public:
	set<pair<int, set<int> > > cl2srccls;
	set<int> visited;
	bool doanalysis_ssy;
	set<ClauseIdx > return_cls_set;
	vector<int> returncls_in1buf;
	int clgrp_ext;//the group of clause that should be extracted

	void do_ssy_analysis(ClauseIdx cidx);
protected:
...
//for incremental SAT
    ...
    void reset (void);
    int	solve(void);
    int solve_ssy(int clgrp2ext,int ** result2ptr2ptr,int * result_size2ptr,int totry);
    int solve_ssy_unsat(int clgrp2ext,int ** result2ptr2ptr,int * result_size2ptr,int totry);
    //int	solve_allsat(void);
