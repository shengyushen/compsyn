...
//add additional cycle to delay it to align with data
wire [3:0]	tx_enc_ctrl_sel_reg;	// encoder control from tx ctrl
...
wire [9:0] 	tx_10bdata_bigend;		// symbol to send over link
...
REG #(10) tx_10bdata_REG (.qout(tx_10bdata_bigend),.clk(txclk),.din(tx_10bdata_predel));
assign tx_10bdata [9] = tx_10bdata_bigend [0] ;
assign tx_10bdata [8] = tx_10bdata_bigend [1] ;
assign tx_10bdata [7] = tx_10bdata_bigend [2] ;
assign tx_10bdata [6] = tx_10bdata_bigend [3] ;
assign tx_10bdata [5] = tx_10bdata_bigend [4] ;
assign tx_10bdata [4] = tx_10bdata_bigend [5] ;
assign tx_10bdata [3] = tx_10bdata_bigend [6] ;
assign tx_10bdata [2] = tx_10bdata_bigend [7] ;
assign tx_10bdata [1] = tx_10bdata_bigend [8] ;
assign tx_10bdata [0] = tx_10bdata_bigend [9] ;

REG #(4) tx_enc_ctrl_sel_reg_delay (.qout(tx_enc_ctrl_sel_reg),.clk(txclk),.din(tx_enc_ctrl_sel));
/*
...
MUX2TO1 #(4) r_tx_enc_sel(.dout(tx_enc_sel),.sel(link_up_loc),
		.data0(tx_enc_conf_sel),.data1(tx_enc_ctrl_sel_reg));
...
assign	assertion_shengyushen=
	...
	//accoring to Table 36–3—Defined ordered_sets of 802.3-2012_section3.pdf
	//PCS_ENC_LINK_CONFA are used to send D21.5
	//PCS_ENC_LINK_CONFB are used to send D2.2
	//PCS_ENC_IDLE1 are used to send D5.6
	//PCS_ENC_IDLE2 are used to send D16.2
	//so their definition are duplicated with PCS_ENC_DATA, 
	//so we dont need them here
	//but the generated decoder will still decode them correctly
//	(tx_enc_ctrl_sel==`PCS_ENC_K285||tx_enc_ctrl_sel==`PCS_ENC_SOP||tx_enc_ctrl_sel==`PCS_ENC_DATA||tx_enc_ctrl_sel==`PCS_ENC_T_CHAR||tx_enc_ctrl_sel==`PCS_ENC_R_CHAR ||tx_enc_ctrl_sel==`PCS_ENC_H_CHAR )// &&
	(tx_enc_ctrl_sel==`PCS_ENC_K285 || tx_enc_ctrl_sel==`PCS_ENC_DATA )// &&
//these are signals not involved in encoding, such as config regs and inputs, they can lead to free run regs that can be both loop and non-loop, so I must rule them out
//	 (adver_reg==13'b0) && !ack && (tx_enc_conf_sel==4'b0) 
	;
